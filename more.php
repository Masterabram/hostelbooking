<?php include 'header.php'; ?>
		...
		<div id="main" class="shell">
			...
			<div id="products">
				<h2>Search Results				           
					<P> <form  action="search.php" method="post" autocomplete="on"> 
                                   Location  <input type="text"  name="location" required="required" type="text" value='<?php echo $_GET['location'];?>'  /> 
                                    Price less than<input type="text" name='price' required="required" type="text"  value='<?php echo $_GET['price'];?>' /> 
	...
	$results = $mysqli->query("SELECT * FROM hostel WHERE (location='{$_GET['location']}' AND price <= '{$_GET['price']}'  AND  qty!=bkqty) ");
    if ($results) { 
