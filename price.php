<?php include 'header.php'; ?>
    ...
    $price = $_GET['price'];
    if ($price=='<10'){
    	$priceSql = "SELECT * FROM hostel WHERE (price <=10000 AND qty!=bkqty)";
    }elseif ($price=='10-20') {
    	$priceSql = "SELECT * FROM hostel WHERE ( price > 20000 AND price <= 30000 AND qty!=bkqty )";
    }elseif ($price=='20-30') {
    	$priceSql = "SELECT * FROM hostel WHERE ( price > 20000 AND price <= 30000 AND qty!=bkqty )";
    }elseif ($price=='30>') {
    	$priceSql = "SELECT * FROM hostel WHERE (price > 30000 AND qty!=bkqty ) ";
    }
	$results = $mysqli->query($priceSql);
    if ($results) { 
